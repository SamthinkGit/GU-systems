...
def test_moving_to_corners(tracker):
    ...
    registry.tools["move_mouse_to"].content(0, 0)
    click_function()
    time.sleep(0.1)
    assert tracker.clicks[-1].side == "blue"

    registry.tools["move_mouse_to"].content(0, INF)
    click_function()
    time.sleep(0.1)
    assert tracker.clicks[-1].side == "blue"

    registry.tools["move_mouse_to"].content(INF, 0)
    click_function()
    time.sleep(0.1)
    assert tracker.clicks[-1].side == "red"

    registry.tools["move_mouse_to"].content(INF, INF)
    click_function()
    time.sleep(0.1)
    assert tracker.clicks[-1].side == "red"
...
def test_mouse_buttons(tracker):
    ...
    registry.load_all()
    registry.tools["send_click_event"].content()
    time.sleep(0.1)
