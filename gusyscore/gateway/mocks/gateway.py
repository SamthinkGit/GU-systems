...
def assert_valid_text(text: str, func: callable) -> None:
    assert len(text) > 0, f"Trying to write text in {func.__name__} that has length 0"

def write(text: str) -> None:
    assert_valid_text(text, func=write)
    pyautogui.write(text)

def write_to_workspace(text: str, workspace: Workspace = Workspace(), temp_file: bool = False, filename: str = None) -> Path:
